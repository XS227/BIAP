...
async function proxyQuote(symbol: MarketSymbolResult): Promise<StockItem | null> {
  const rec = await fetchRecommendation(symbol.symbol || symbol.code, 10_000);
...
async function resolveTsetmcCode(symbol: MarketSymbolResult, timeoutMs = 8_000): Promise<string | null> {
  ...
  try {
    ...
    const exact = rows.find((row:any)=>normalized(String(row?.lVal18AFC??''))===wanted && /^\d+$/.test(String(row?.insCode??''))) ?? rows.find((row:any)=>normalized(String(row?.lVal30??''))===normalized(symbol.name) && /^\d+$/.test(String(row?.insCode??'')));
    const code = exact ? String(exact.insCode) : null; if (code) resolvedCodeCache.set(cacheKey, code); return code;
  } catch { return null; } finally { clearTimeout(timer); }
}

export async function fetchTsetmcQuote(symbol: MarketSymbolResult, timeoutMs = 8_000, allowProxy = true): Promise<StockItem> {
  if (allowProxy) { const proxied = await proxyQuote(symbol); if (proxied) return proxied; }
  ...
  try {
    ...
    const row = (await res.json())?.closingPriceInfo; if (!row || typeof row !== 'object') return { name: symbol.symbol || symbol.name, code: symbol.code, error: true };
    const lastPrice=numberOrUndefined(row.pDrCotVal), closingPrice=numberOrUndefined(row.pClosing), yesterdayPrice=numberOrUndefined(row.priceYesterday); const effective=lastPrice??closingPrice; const change=effective!==undefined&&yesterdayPrice!==undefined?effective-yesterdayPrice:undefined; const changePercent=change!==undefined&&yesterdayPrice?(change/yesterdayPrice)*100:undefined;
    return { name:symbol.symbol||symbol.name, code:symbol.code, lastPrice, closingPrice, yesterdayPrice, change, changePercent };
  } catch { return { name:symbol.symbol||symbol.name, code:symbol.code, error:true }; } finally { clearTimeout(timer); }
}

export async function fetchTsetmcHistory(symbol: MarketSymbolResult, days=60, timeoutMs=10_000): Promise<PricePoint[]> {
  const resolvedCode=await resolveTsetmcCode(symbol,timeoutMs); if(!resolvedCode)return[]; const controller=new AbortController(); const timer=setTimeout(()=>controller.abort(),timeoutMs);
  try{const res=await fetch(`${TSETMC_API_BASE}/ClosingPrice/GetClosingPriceDailyList/${resolvedCode}/${Math.max(days,30)}`,{signal:controller.signal,headers:{Accept:'application/json'}});if(!res.ok)return[];const rows=(await res.json())?.closingPriceDaily;return (Array.isArray(rows)?rows:[]).map((row:any)=>({date:String(row?.dEven??''),close:Number(row?.pClosing??row?.pDrCotVal??0)})).filter((p:PricePoint)=>Number.isFinite(p.close)&&p.close>0).slice(0,days).reverse()}catch{return[]}finally{clearTimeout(timer)}}

export async function fetchTsetmcQuotes(symbols: MarketSymbolResult[]): Promise<Record<string, StockItem>> {
  const output:Record<string,StockItem>={}; for(let i=0;i<symbols.length;i+=8){const chunk=symbols.slice(i,i+8);const results=await Promise.all(chunk.map(s=>fetchTsetmcQuote(s,8_000,false)));results.forEach(q=>{output[q.code]=q})} return output;
}
