...
export default function DataConnectScreen() {
  ...
  const refresh = async () => {
    ...
    setDatasetInfo(d ? `${d.name} • ${d.rows.length} ردیف • ${d.columns.length} ستون` : 'هنوز داده شرکت وارد نشده است');
  };
  useEffect(() => { refresh(); }, []);
  ...
  const importData = async () => {
    try {
      ...
      setStatus(`✓ متصل شد: ${dataset.rows.length} ردیف و ${dataset.columns.length} ستون`);
      setRaw('');
      await refresh();
    } catch (e) {
  ...
  const clear = async () => { await clearBusinessDataset(); setStatus('داده شرکت حذف شد'); await refresh(); };
    ...
    <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={[styles.content, { paddingBottom: BottomTabInset + Spacing.four }]}>
      <View style={styles.wrap}>
        ...
        <View style={[styles.activeCard, { backgroundColor: colors.backgroundElement }]}>
          ...
          <Text style={[styles.body, { color: colors.textSecondary }]}>Market/CODAL/Kiasha فعال هستند. داده اختصاصی شرکت هم اکنون می‌تواند به‌صورت CSV یا JSON وارد شود و مستقیم در Real Mode ماژول‌ها استفاده شود.</Text>
          <View style={styles.liveRow}><Text style={[styles.liveItem, { color: Brand.positive }]}>● Company Dataset</Text><Text style={[styles.liveItem, { color: Brand.positive }]}>● Market / CODAL / Kiasha</Text></View>
        </View>

        <View style={[styles.importCard, { backgroundColor: colors.backgroundElement }]}>
          <View style={styles.activeHead}><View style={styles.liveBadge}><Text style={styles.liveBadgeText}>READY</Text></View><Text style={[styles.activeTitle, { color: colors.text }]}>CSV / JSON شرکت</Text></View>
          <Text style={[styles.body, { color: colors.textSecondary }]}>محتوای CSV را با ردیف عنوان، یا آرایه JSON از رکوردها وارد کنید. این داده فقط برای تحلیل واقعی استفاده می‌شود و Demo نیست.</Text>
          ...
          <TextInput value={raw} onChangeText={setRaw} multiline textAlignVertical="top" placeholder={'مثال:\nmonth,revenue,cost,customers\n1405-01,1200000,700000,240'} placeholderTextColor={colors.textSecondary} style={[styles.area, { color: colors.text, borderColor: colors.backgroundSelected }]} />
          <Pressable onPress={importData} style={styles.primary}><Text style={styles.primaryText}>اتصال و تحلیل داده</Text></Pressable>
        ...
        <Text style={[styles.sectionNote, { color: colors.textSecondary }]}>این سه کانکتور به credential واقعی سازمان نیاز دارند؛ بدون مشخصات منبع نمی‌توان آنها را جعلی LIVE نشان داد.</Text>
