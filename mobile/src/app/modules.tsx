...
function ModuleCard({ item, colors, accent, state }: { item: ModuleItem; colors: ThemeColors; accent: string; state: string }) {
  ...
  return <Pressable onPress={open} style={({ pressed }) => [styles.moduleCard, { backgroundColor: colors.backgroundElement, opacity: pressed ? 0.75 : 1 }]}>
    ...
    <Text style={[styles.state, { color: state === 'LIVE' ? Brand.stockGreen : state === 'DEMO' ? '#a78bfa' : colors.textSecondary }]}>{state}</Text>
  </Pressable>;
}
...
export default function ModulesScreen() {
  ...
  const refreshStatus = async () => {
    const [d, dataset] = await Promise.all([getDemoMode(), getBusinessDataset()]);
    setDemo(d); setCompanyConnected(Boolean(dataset?.rows.length));
  };
  ...
  return <SafeAreaView style={[styles.safe, { backgroundColor: colors.background }]}><ScrollView contentContainerStyle={[styles.content, { paddingBottom: BottomTabInset + Spacing.six }]}><View style={styles.maxWidth}>
    ...
    <View style={[styles.controlCard,{backgroundColor:colors.backgroundElement}]}>
      <View style={{flex:1,alignItems:'flex-end'}}><Text style={[styles.controlTitle,{color:colors.text}]}>حالت ماژول‌ها: {demo?'Demo':'Real'}</Text><Text style={[styles.controlSub,{color:colors.textSecondary}]}>{companyConnected?'داده شرکت متصل است.':'داده شرکت هنوز متصل نیست.'}</Text></View>
    ...
    <Pressable onPress={() => router.push('/data-connect' as never)} style={[styles.connect,{borderColor:companyConnected?Brand.stockGreen:Brand.primary}]}><Text style={[styles.connectText,{color:companyConnected?Brand.stockGreen:Brand.primary}]}>{companyConnected?'مدیریت داده متصل':'اتصال داده شرکت'}</Text></Pressable>
...
const styles = StyleSheet.create({
  ...
  hero:{borderRadius:Radius.lg,padding:Spacing.four,marginBottom:Spacing.three},heroMark:{color:'#8ab4ff',fontFamily:Fonts.mono,fontSize:12,fontWeight:'800',marginBottom:8},heroTitle:{fontFamily:Fonts.sans,fontSize:19,fontWeight:'800',textAlign:'right',lineHeight:30},heroBody:{fontFamily:Fonts.sans,fontSize:12,textAlign:'right',lineHeight:21,marginTop:6},controlCard:{flexDirection:'row-reverse',alignItems:'center',gap:Spacing.three,borderRadius:Radius.md,padding:Spacing.three},controlTitle:{fontFamily:Fonts.sans,fontSize:14,fontWeight:'900'},controlSub:{fontFamily:Fonts.sans,fontSize:10.5,marginTop:3},modeButton:{paddingHorizontal:14,paddingVertical:9,borderRadius:16},modeButtonText:{color:'#fff',fontFamily:Fonts.mono,fontSize:10,fontWeight:'900'},connect:{borderWidth:1,borderRadius:Radius.md,paddingVertical:11,alignItems:'center',marginTop:Spacing.two,marginBottom:Spacing.four},connectText:{fontFamily:Fonts.sans,fontSize:12,fontWeight:'900'},
