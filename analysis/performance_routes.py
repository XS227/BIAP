...
class AutoInvestSettingsRequest(BaseModel):
    enabled:bool;horizon:Literal["short","long"]="short";maxDailyTrades:int=Field(default=3,ge=1,le=3)
...
def market_history(code:str,days:int=Query(default=90,ge=5,le=400)):
    ...
    rows=payload.get("closingPriceDaily");points=[]
...
@router.get("/ai/kiasha-equity-history")
def ai_kiasha_equity_history(limit:int=Query(default=400,ge=1,le=2000),user_id:str=Depends(require_user_id)):
    user_id=str(user_id);m=KIASHA_CAPITAL_STORE.active_mandate(user_id=user_id)
    if m:
        prices={};complete=True
        for p in m.get("positions",[]):
            if int(p.get("quantity") or 0)<=0:continue
            price,_,_=_verified_reference_price(str(p["code"]));
            if price is None:complete=False;break
            prices[str(p["code"]).upper()]=price
        if complete:KIASHA_CAPITAL_STORE.record_verified_equity_snapshot(user_id=user_id,prices=prices)
    items=KIASHA_CAPITAL_STORE.list_equity_snapshots(user_id=user_id,limit=limit);return {"items":items,"count":len(items),"valuation":"verified-market-marks-only"}
...
def ai_paper_dry_run(code:str,horizon:Literal["short","long"]=Query(default="short"),user_id:str=Depends(require_user_id)):
    user_id=str(user_id);mandate=KIASHA_CAPITAL_STORE.active_mandate(user_id=user_id);sizing=float(mandate["accountingEquityAtCost"]) if mandate else 0.;proposal=_run_ai_analysis(code,horizon);price,source,fetched=_verified_reference_price(code);owned=next((float(p["quantity"]) for p in (mandate or {}).get("positions",[]) if p["code"].upper()==code.upper()),0.);result=evaluate_ai_paper_proposal(proposal,portfolio_value=sizing,reference_price=price,current_symbol_position=owned,max_position_pct=5.,quote_fetched_at=fetched,execute=False);payload=result.to_dict();payload.update({"dryRun":True,"kiashaMandateSizingCapital":sizing,"referencePrice":price,"referencePriceSource":source,"paperExecution":False,"liveExecution":False,"requiresCapitalMandate":mandate is None});return payload
...
def ai_paper_execute(code:str,horizon:Literal["short","long"]=Query(default="short"),idempotency_key:str=Header(...,alias="Idempotency-Key",min_length=8,max_length=128),user_id:str=Depends(require_user_id)):
    ...
    mandate=KIASHA_CAPITAL_STORE.active_mandate(user_id=user_id)
    ...
    sizing=float(mandate["accountingEquityAtCost"]);proposal=_run_ai_analysis(code,horizon);price,source,fetched=_verified_reference_price(code);owned=next((float(p["quantity"]) for p in mandate.get("positions",[]) if p["code"].upper()==code.upper()),0.);result=evaluate_ai_paper_proposal(proposal,portfolio_value=sizing,reference_price=price,current_symbol_position=owned,max_position_pct=5.,quote_fetched_at=fetched,execute=False);base=result.to_dict();base.update({"dryRun":False,"kiashaMandateSizingCapital":sizing,"referencePrice":price,"referencePriceSource":source,"paperExecution":False,"liveExecution":False})
    if not result.allowed or result.intent is None or result.risk is None:return base
...
def market_symbols(market:Optional[str]=Query(default=None),q:Optional[str]=Query(default=None,max_length=64),limit:int=Query(default=5000,ge=1,le=10000)):
    ...
    return {"count":len(items),"markets":["TSE","IFB","IFB_BASE"],"items":[item.to_dict() for item in items]}
