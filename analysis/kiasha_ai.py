...
def status() -> dict[str, Any]:
    return {"provider":"anthropic","model":DEFAULT_MODEL,"configured":bool(os.getenv("ANTHROPIC_API_KEY")),"proposalOnly":True,"paperExecution":False,"liveExecution":False,"maxPositionPct":MAX_POSITION_PCT}


def _verified_company(code: str) -> tuple[dict[str, Any], str]:
    try: quote = find_quote(code)
    except MarketDataUnavailable: quote = None
    if quote is not None: return build_company_from_quote(quote, codal_symbol=quote.name), "live"
    company = build_company_from_symbol(code)
    if company is not None: return company, "codal"
...
def _market_tool(company: dict[str, Any], source: str) -> dict[str, Any]:
    ...
    return {"source":source,"ticker":company.get("ticker"),"name":company.get("name_fa"),"price":market.get("price"),"lastPrice":market.get("last_price"),"closingPrice":market.get("closing_price"),"yesterdayPrice":market.get("yesterday_price"),"changePercent":market.get("change_percent"),"dayLow":market.get("day_low"),"dayHigh":market.get("day_high"),"volumeToday":market.get("volume_today"),"avgVolume30d":market.get("avg_volume_30d"),"pe":market.get("pe"),"sectorAvgPe":market.get("sector_avg_pe"),"marketCap":market.get("market_cap"),"availability":availability(company)}


def _codal_tool(company: dict[str, Any], source: str) -> dict[str, Any]:
    return {"source":source,"ticker":company.get("ticker"),"metadata":company.get("codal_metadata"),"fundamentals":company.get("codal"),"availability":availability(company)}


def _team_tool(company: dict[str, Any]) -> dict[str, Any]:
    d=decide(company);return {"call":d.call,"weightedScore":d.weighted_score,"explanation":d.explanation,"breakdown":d.breakdown}
...
def _headers(api_key:str)->dict[str,str]:return {"x-api-key":api_key,"anthropic-version":ANTHROPIC_VERSION,"content-type":"application/json"}

def _tool_result(name:str,company:dict[str,Any],source:str)->dict[str,Any]:
    if name=="get_market_snapshot":return _market_tool(company,source)
    if name=="get_codal_fundamentals":return _codal_tool(company,source)
    if name=="get_kiasha_team_signal":return _team_tool(company)
...
def _validated_proposal(code:str,horizon:Horizon,model:str,raw:dict[str,Any])->KiashaAIProposal:
    ...
    if action not in {"BUY","HOLD","SELL"}:raise ValueError("invalid action from AI")
    confidence=max(0.0,min(1.0,float(raw.get("confidence",0))))
    position_pct=max(0.0,min(MAX_POSITION_PCT,float(raw.get("positionPct",0))))
    if action!="BUY":position_pct=0.0
    elif position_pct<=0:raise ValueError("BUY positionPct must be positive")
    thesis=str(raw.get("thesis") or "").strip()
    if not thesis:raise ValueError("AI proposal thesis is required")
...
def _extract_final_proposal(blocks:list[dict[str,Any]],code:str,horizon:Horizon,model:str)->KiashaAIProposal|None:
    for block in blocks:
        if block.get("type")=="tool_use" and block.get("name")=="propose_investment":
            raw=block.get("input")
            if isinstance(raw,dict):return _validated_proposal(code,horizon,model,raw)
    return None
...
def propose(code:str,*,horizon:Horizon="short",max_rounds:int=6,client:httpx.Client|None=None)->KiashaAIProposal:
    ...
    if not api_key:raise RuntimeError("ANTHROPIC_API_KEY is not configured")
    company,source=_verified_company(code);model=DEFAULT_MODEL;own_client=client is None;http=client or httpx.Client(timeout=DEFAULT_TIMEOUT)
    ...
    try:
        for _ in range(max_rounds):
            payload=_request(http,api_key=api_key,model=model,messages=messages,max_tokens=1800);blocks=payload.get("content")
            if not isinstance(blocks,list):raise RuntimeError("Anthropic response content is invalid")
            ...
            if not tool_uses:raise RuntimeError("Kiasha AI stopped without a final proposal")
            messages.append({"role":"assistant","content":blocks});tool_results=[]
            for call in tool_uses:
                tool_name=str(call.get("name") or "");tool_id=str(call.get("id") or "")
                if tool_name=="propose_investment":continue
                result=_tool_result(tool_name,company,source);tool_results.append({"type":"tool_result","tool_use_id":tool_id,"content":json.dumps(result,ensure_ascii=False)})
            if not tool_results:raise RuntimeError("Kiasha AI returned no executable analysis tool calls")
            messages.append({"role":"user","content":tool_results})
        raise RuntimeError("Kiasha AI exceeded the maximum tool rounds")
    finally:
        if own_client:http.close()


def analyze(code:str,*,horizon:Horizon="short",max_rounds:int=6,client:httpx.Client|None=None)->KiashaAIProposal:
