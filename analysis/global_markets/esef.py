...
def _period_parts(raw: Any) -> tuple[Optional[date], Optional[date]]:
    ...
    try:
        ...
        start = date.fromisoformat(parts[0][:10])
...
def _unit_currency(raw: Any) -> Optional[str]:
    text = str(raw or "")
    match = re.search(r"(?:iso4217:|currency:)([A-Z]{3})", text, re.IGNORECASE)
...
class ESEFFundamentalsProvider(FundamentalsProvider):
    ...
    def _get_json(self, url: str, *, params: Optional[dict] = None) -> dict:
        try:
            with httpx.Client(
                ...
                headers={"Accept": "application/json", "User-Agent": "BIAP-Global/1.0 ESEF evidence"},
            ) as client:
    ...
    def _resolve_lei(self, company: GlobalCompany) -> tuple[str, str]:
        ...
        # A ticker is never used for entity matching. Only the full company name
        # may be resolved, and only exact unambiguous GLEIF legal-name matches pass.
    ...
    def _latest_filing(self, lei: str, country: str) -> dict:
        ...
        for row in rows:
            ...
            # The same legal entity may file on multiple markets. Prefer the
            # selected country's filing, but keep cross-market fallback if none.
    ...
    def _facts(payload: dict) -> list[dict]:
        ...
        if not isinstance(raw, dict):
            return []
        return [fact for fact in raw.values() if isinstance(fact, dict)]

    @staticmethod
    def _plain_fact(fact: dict) -> bool:
        dims = fact.get("dimensions") if isinstance(fact.get("dimensions"), dict) else {}
        # Headline metrics must not come from segment/member facts.
    ...
    def _concept_facts(cls, facts: list[dict], concepts: tuple[str, ...]) -> list[dict]:
        ...
        for fact in facts:
            dims = fact.get("dimensions") if isinstance(fact.get("dimensions"), dict) else {}
            concept = str(dims.get("concept") or "").lower()
            if concept not in wanted or not cls._plain_fact(fact) or _num(fact.get("value")) is None:
                continue
            result.append(fact)
        return result
    ...
    def _duration_value(facts: list[dict], period_end: date, *, previous: bool = False) -> Optional[float]:
        ...
        ordered = sorted(unique_by_end.values(), key=lambda row: row[1], reverse=True)
        current = [row for row in ordered if row[1] <= period_end]
        index = 1 if previous else 0
        return current[index][2] if len(current) > index else None
    ...
    def enrich_fundamentals(self, company: GlobalCompany) -> GlobalCompany:
        ...
        return append_source(enriched, SourceEvidence(
            ...
            quality=quality,
            notes=(
                f"ESEF/UKSEF xBRL-JSON indexed by filings.xbrl.org; LEI={lei}; "
                f"validationErrors={errors}; inconsistencies={inconsistencies}"
            ),
        ))
