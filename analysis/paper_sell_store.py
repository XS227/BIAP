...
class PaperSellStore:
    ...
    @staticmethod
    def _mandate_schema_present(conn): return conn.execute("SELECT 1 FROM sqlite_master WHERE type='table' AND name='kiasha_capital_mandates'").fetchone() is not None
    @staticmethod
    def _active_mandate(conn,user_id):
        if not PaperSellStore._mandate_schema_present(conn): return None
        return conn.execute("SELECT * FROM kiasha_capital_mandates WHERE user_id=? AND status IN('ACTIVE','STOPPING') ORDER BY created_at DESC LIMIT 1",(user_id,)).fetchone()
    def commit_sell_fill(self,*,user_id:str,code:str,horizon:str,proposal:dict[str,Any],risk:dict[str,Any],intent:dict[str,Any],receipt:dict[str,Any],reference_price:float,reference_source:str,idempotency_key:str)->dict[str,Any]:
        ...
        try:
            ...
            owned=int(position["quantity"]); schema_present=self._mandate_schema_present(conn); mandate=self._active_mandate(conn,user_id); mandate_id=None; mandate_qty=0
            ...
            owner="manual" if manual else ("kiasha" if mandate_id is not None else "legacy-paper")
            persisted={**receipt,"status":"PAPER_FILLED"}
            existing_intent=conn.execute("SELECT user_id FROM order_intents WHERE id=?",(intent["id"],)).fetchone()
            if existing_intent is None:
                conn.execute("INSERT INTO order_intents(id,user_id,code,side,quantity,limit_price,mode,status,recommendation_call,recommendation_score,created_at,updated_at,payload_json) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)",(intent["id"],user_id,intent["code"],"SELL",quantity,intent.get("limit_price"),"paper","PAPER_FILLED",intent["recommendation_call"],intent["recommendation_score"],intent["created_at"],now,json.dumps(persisted,ensure_ascii=False,sort_keys=True)))
            elif str(existing_intent["user_id"])!=str(user_id):
                raise ValueError("Paper intent belongs to another user")
            else:
                conn.execute("UPDATE order_intents SET status=?,updated_at=?,payload_json=? WHERE id=? AND user_id=?",("PAPER_FILLED",now,json.dumps(persisted,ensure_ascii=False,sort_keys=True),intent["id"],user_id))
            decision_id=f"kai_{uuid4().hex}"; result={"allowed":True,"reasons":[],"proposal":proposal,"risk":risk,"intent":intent,"receipt":persisted,"paperExecution":True,"liveExecution":False,"dryRun":False,"decisionId":decision_id,"referencePrice":price,"referencePriceSource":reference_source,"fillProceeds":proceeds,"realizedPnL":realized_pnl,"executionOwner":owner,"kiashaMandateId":mandate_id,"accountAfter":{"userId":user_id,"initialCash":float(account["initial_cash"]),"cashBalance":cash_after,"soldCode":symbol,"remainingQuantity":remaining}}
            ...
            event_type="MANUAL_PAPER_SOLD" if manual else "KIASHA_AI_PAPER_SOLD"; payload={"decisionId":decision_id,"code":symbol,"horizon":horizon,"quantity":quantity,"price":price,"proceeds":proceeds,"realizedPnL":realized_pnl,"executionOwner":owner,"kiashaMandateId":mandate_id,"cashBefore":cash_before,"cashAfter":cash_after,"remainingQuantity":remaining,"broker":"paper"}
