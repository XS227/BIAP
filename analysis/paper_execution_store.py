...
class PaperExecutionStore:
    ...
    @staticmethod
    def _mandate_schema_present(conn):
        return conn.execute("SELECT 1 FROM sqlite_master WHERE type='table' AND name='kiasha_capital_mandates'").fetchone() is not None
    @staticmethod
    def _active_mandate(conn,user_id):
        if not PaperExecutionStore._mandate_schema_present(conn): return None
    ...
    def commit_buy_fill(self,*,user_id:str,code:str,horizon:str,proposal:dict[str,Any],risk:dict[str,Any],intent:dict[str,Any],receipt:dict[str,Any],reference_price:float,reference_source:str,idempotency_key:str)->dict[str,Any]:
        ...
        try:
            ...
            schema_present=self._mandate_schema_present(conn); mandate=self._active_mandate(conn,user_id); mandate_id=None
            if manual:
                reserved=float(mandate["mandate_cash"]) if mandate is not None else 0.0; available=max(0.0,cash_before-reserved)
