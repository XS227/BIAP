...
HEADERS = {"User-Agent": "BIAP-Global-FIRDS-join-probe-v2/1.0 (+https://setai.no)"}

MARKETS = {
    "FR": {"exchange": "EURONEXT_PARIS", "native_mics": {"XPAR"}, "currency": "EUR"},
    "IT": {"exchange": "EURONEXT_MILAN", "native_mics": {"MTAA"}, "currency": "EUR"},
    "NL": {"exchange": "EURONEXT_AMSTERDAM", "native_mics": {"XAMS"}, "currency": "EUR"},
    "BE": {"exchange": "EURONEXT_BRUSSELS", "native_mics": {"XBRU"}, "currency": "EUR"},
    "PT": {"exchange": "EURONEXT_LISBON", "native_mics": {"XLIS"}, "currency": "EUR"},
    "NO": {"exchange": "EURONEXT_OSLO", "native_mics": {"XOSL"}, "currency": "NOK"},
    "ES": {"exchange": "BME_MADRID", "native_mics": {"XMAD"}, "currency": "EUR"},
}
            ...
            with zf.open(xml_name) as fh:
                for _, elem in ET.iterparse(fh, events=("end",)):
                    ...
                    if not country:
                        elem.clear(); continue
                    cfg = MARKETS[country]
                    if relevant != actual or not cfi.startswith("ES") or currency != cfg["currency"] or not isin:
                        elem.clear(); continue
                    output[country][isin] = {
                        "isin": isin,
    ...
    for country, cfg in MARKETS.items():
        ...
        for isin, identity in official[country].items():
            ...
            if len(matches) == 1:
                ...
                exact = [row for row in matches if row.mic_code in cfg["native_mics"]]
                if len(exact) == 1:
                    row = exact[0]
                    joined.append({**identity, "ticker": row.ticker, "vendorMic": row.mic_code, "vendorName": row.name})
                else:
        ...
        report[country] = {
            ...
            "ambiguous": ambiguous,
            "sampleJoined": joined[:12],
            "vendorIsinPrefixTop": Counter((row.isin or "")[:2] for row in vendor_with_isin).most_common(12),
        }
