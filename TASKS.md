...
1. Pull latest main in mobile checkout and run `cd mobile && npx tsc --noEmit`.
2. Expo device test: home should no longer be limited to three fixed symbols; verify REAL MARKET rows, faster price loading, resilient symbol logos, Kiasha short/long horizon picks, Paper Track Record, `/kiasha-profile`, and `/how-to` from More menu.
3. Complete Nasrin production signup/login verification for `nasrin.dadashi@gmail.com`.
4. Contact Farabi at 1561 and request official partner API docs, sandbox/UAT credentials, account linking, buying power/portfolio, order/fill APIs and hosted deposit/funding flow.
