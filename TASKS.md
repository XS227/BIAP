...
`[IN PROGRESS] Kiasha daily Paper Auto Invest — owner: ChatGPT session — since: 2026-08-29 — scope/status:` Added `analysis/kiasha_auto_invest.py` with explicit per-user opt-in settings, Tehran trading-day/window guard, once-per-user/day run claim, server-owned Paper cash/position sizing, verified BIAP candidate ranking, bounded Claude review of the strongest BUY candidates, existing deterministic Paper risk gate, atomic/idempotent Paper fills, and no live-broker path. Added authenticated GET/PUT `/performance/ai/auto-invest` plus POST `/performance/ai/auto-invest/run-now`, a one-shot `analysis/run_kiasha_auto_invest.py` entrypoint for a systemd timer, store tests, and a Kiasha mobile `Auto Invest — Paper` switch/status card. Auto Invest defaults OFF per user and both `KIASHA_AUTO_INVEST_RUNNER_ENABLED` and `KIASHA_PAPER_EXECUTION_ENABLED` remain fail-closed until deployment verification. Current autonomous execution is BUY/HOLD-by-no-trade; SELL/rebalancing remains a separate next safety extension. Commits: `c12d1fe`, `848eb7c`, `be2eaf9`, `4533339`, `85b8d24`, `1b8a2d7`. Next: pull latest, run Python compile/tests + mobile TypeScript, then install/enable the server timer and Paper-only flags in a controlled test account before arming Auto Invest in the app.
...
1. Pull latest main; run Python compile/tests for Auto Invest and mobile `npx tsc --noEmit` before deploying the runner.
2. Keep `ANTHROPIC_API_KEY` server-only and `LIVE_TRADING_ENABLED=false`; never expose the key in Expo/mobile.
3. After tests pass, enable only `KIASHA_PAPER_EXECUTION_ENABLED=true` and `KIASHA_AUTO_INVEST_RUNNER_ENABLED=true` on the FIN server, install a periodic systemd timer for `analysis/run_kiasha_auto_invest.py`, then verify on one authenticated Paper account before broader use.
