...
1. Pull latest main on the running FIN checkout and run `cd analysis && .venv/bin/python -m pytest -q test_kiasha_ai.py test_broker_runtime.py test_kiasha_paper.py` plus `.venv/bin/python -m py_compile kiasha_ai.py kiasha_paper.py performance_routes.py`.
2. Keep `ANTHROPIC_API_KEY` server-only and `LIVE_TRADING_ENABLED=false`; never expose the key in Expo/mobile.
3. After Paper-gate tests pass, add an authenticated dry-run API that calls Claude proposal -> deterministic Paper gate with `execute=false`; do not enable automatic Paper fills until dry-run output/audit is reviewed.
