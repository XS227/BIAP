...
1. On the running FIN checkout, pull latest main and run `cd analysis && pytest -q test_kiasha_ai.py test_broker_runtime.py` plus `python3 -m py_compile kiasha_ai.py performance_routes.py`.
2. Configure `ANTHROPIC_API_KEY` server-side from `.env.ai.example` and keep `LIVE_TRADING_ENABLED=false`; never expose the key in Expo/mobile.
3. Restart `biap-fin`, confirm `/api/performance/ai/status` reports configured=true/proposalOnly=true, then test one authenticated `/api/performance/ai/analyze/{code}` call before starting Paper execution.
4. Expo device test: verify REAL MARKET rows, faster price loading, resilient symbol logos, Kiasha short/long horizon picks, Paper Track Record, `/kiasha-profile`, and `/how-to` from More menu.
