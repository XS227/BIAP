...
`[IN PROGRESS] Kiasha daily Paper Auto Invest — owner: ChatGPT session — since: 2026-08-29 — scope/status:` Added `analysis/kiasha_auto_invest.py` with explicit per-user opt-in settings, Tehran trading-day/window guard, once-per-user/day run claim, server-owned Paper cash/position sizing, verified BIAP candidate ranking, bounded Claude review of the strongest BUY candidates, existing deterministic Paper risk gate, atomic/idempotent Paper fills, and no live-broker path. Added authenticated GET/PUT `/performance/ai/auto-invest` plus POST `/performance/ai/auto-invest/run-now`, a one-shot `analysis/run_kiasha_auto_invest.py` entrypoint, store tests, Kiasha mobile `Auto Invest — Paper` switch/status card, and repo-managed systemd unit templates at `deploy/systemd/biap-kiasha-auto-invest.service` + `.timer`. Scheduled Auto Invest now starts at 09:00 Tehran to match the deterministic risk-session opening and avoid consuming the once-per-day claim before trading opens. Server verification passed 12/12 Auto/Paper tests and mobile `npx tsc --noEmit`; runtime verification now confirms `KIASHA_PAPER_EXECUTION_ENABLED=true`, `KIASHA_AUTO_INVEST_RUNNER_ENABLED=true`, `LIVE_TRADING_ENABLED=false`, timer active, FIN active, and one-shot timer execution exits cleanly. Added a safe public readiness surface (`/performance/ai/status`) for runner + Paper flags and mobile fallback so expired/missing JWT no longer masquerades as “server disabled”; authenticated per-user settings still remain protected. Auto Invest defaults OFF per user. Current autonomous execution is BUY/HOLD-by-no-trade; SELL/rebalancing remains a separate next safety extension. Relevant commits include `c12d1fe`, `848eb7c`, `be2eaf9`, `4533339`, `85b8d24`, `1b8a2d7`, `dd63bac`, `00837c0`, `4e78f1e`, `951dee7`, `e759167`, `1d56ad0`. Next: deploy latest backend/mobile, refresh login if JWT is stale, verify one authenticated Paper account can arm Auto Invest, then confirm Paper account/order visibility.
...
1. Pull latest main, restart `biap-fin`, and reload Expo so public runner/Paper readiness and auth-fallback logic are live.
2. Keep `ANTHROPIC_API_KEY` server-only and `LIVE_TRADING_ENABLED=false`; never expose the key in Expo/mobile.
3. If mobile reports server ready but arming still fails, refresh login/JWT and verify `/performance/ai/auto-invest` on one authenticated Paper account; Portfolio and Orders are likewise authenticated per-user views.
