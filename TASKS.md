...
`[DONE] Align mobile registration with live Express auth contract — owner: ChatGPT session — since: 2026-08-28 — scope/result:` Verified production Express auth live on `89.42.199.20`: registration is `POST /api/auth/signup` with `fullName`, email and password (minimum 8 chars), login is `POST /api/auth/login`, and both return the existing JWT `accessToken` contract. Updated `mobile/src/app/register.tsx` in commit `a188193` to use `/auth/signup`, send `fullName`, enforce the backend's 8-character password minimum, normalize email/name, and continue storing `accessToken`/`user` under the same AsyncStorage keys already consumed by `mobile/src/lib/api.ts` and `login-screen.tsx`. Live demo account signup/login was verified manually before the code change. Remaining verification: pull latest main on the mobile VPS, run `npx tsc --noEmit`, then test signup/login in Expo.
