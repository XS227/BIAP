...
1. Expo device test: verify Real Trade card appears on stock detail for a normal/non-demo account, shows Farabi selected plus completed/pending readiness checklist, and exposes no real order action.
2. Complete Nasrin production signup/login verification for `nasrin.dadashi@gmail.com`.
3. Contact Farabi at 1561 and ask for the technical/business unit handling official third-party trading APIs/algorithmic-trading platform integrations. Request partner API docs, sandbox/UAT credentials, account linking, buying power/portfolio, order/fill APIs and official hosted deposit/funding flow.
